...
function init(){
  ...
  L.control.layers({'Standard':standard,'Topographic':topographic}).addTo(map);

  // IMPORTANT: featureGroup has clearLayers() and getBounds().
  markers=L.featureGroup().addTo(map);

  const legend=L.control({position:'bottomleft'});
  legend.onAdd=()=>{
    const div=L.DomUtil.create('div','legend');
    div.innerHTML='<b>Rule:</b> live circles are API nodes.<br>Actual boundaries appear only from verified GeoJSON.<br>No fake polygons.';
    return div;
  };
  legend.addTo(map);
}
