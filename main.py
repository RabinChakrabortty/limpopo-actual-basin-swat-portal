...
function init(){map=L.map('map').setView([-23.7,30.1],6);let a=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors'}).addTo(map),b=L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png',{attribution:'© OpenTopoMap'});L.control.layers({'Standard':a,'Topographic':b}).addTo(map);let x=L.control({position:'bottomleft'});x.onAdd=()=>{let d=L.DomUtil.create('div','legend');d.innerHTML='<b>Rule:</b> live circles are API nodes.<br>Actual boundaries appear only from verified GeoJSON.<br>No fake polygons.';return d};x.addTo(map)}
